...
class AsyncTTSClient:
    ...
    async def generate(self, req: TTSRequest) -> AsyncIterator[TTSChunk]:
        ...
        # --- keep header and body IDs in sync ---
        rid = req.request_id or f"tts-{uuid.uuid4()}"
        ...
        payload = {
            ...
            "request_id": rid,
            "timeout_s": float(req.timeout_s or 60.0),  # <— send timeout_s explicitly
        }
        ...
        try:
            async with self.client.stream("POST", url, headers=headers, json=payload) as r:
                ...
                async for line in r.aiter_lines():
                    ...
                    if typ == "chunk":
                        ...
                        # Surface gateway/worker errors (e.g. "rate_limited", "deadline_exceeded")
                        msg = obj.get("message", "unknown_error")
                        raise RuntimeError(f"TTS stream error from server: {msg}")

                    # deadline check regardless of frame type
            ...
            # retry once if no bytes were received
            if timings.chunks == 0:
