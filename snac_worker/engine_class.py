...
class OrpheusModel:
    def __init__(self, model_name, dtype=torch.half, tokenizer='canopylabs/orpheus-3b-0.1-pretrained', **engine_kwargs):
        self.model_name = "/home/ubuntu/Orpheus/Orpheus/orpheus_unsloth/hf_cache"
        self.dtype = dtype
        self.engine_kwargs = engine_kwargs
        self.engine = self._setup_engine()
        self.available_voices = ["zoe", "zac", "jess", "leo", "mia", "julia", "leah"]
        self.tokenizer = self._load_tokenizer("/home/ubuntu/Orpheus/Orpheus/orpheus_unsloth/hf_cache")

    def _load_tokenizer(self, tokenizer_path):
        try:
            if tokenizer_path is None: tokenizer_path = self.model_name
    ...
    def _setup_engine(self):
        engine_args = AsyncEngineArgs(
            model="/home/ubuntu/Orpheus/Orpheus/orpheus_unsloth/hf_cache",
            tokenizer="/home/ubuntu/Orpheus/Orpheus/orpheus_unsloth/hf_cache",
            dtype=torch.float16, max_model_len=512, gpu_memory_utilization=0.5,
            load_format="auto",
    ...
    # ===== CORRECTED ASYNC-NATIVE TOKEN GENERATOR =====
    async def generate_tokens_async(self, prompt, voice=None, request_id="req-001", **kwargs):
        """An async generator that yields ONLY THE NEWLY-GENERATED text (delta) from vLLM."""
        ...
        async for result in results_generator:
            # The result.outputs[0].text is the CUMULATIVE text.
            current_text = result.outputs[0].text
            
            # This is the critical change: find the new part of the string.
            newly_generated_text = current_text[len(previous_text):]
            
            if newly_generated_text:
                # This debug print now correctly shows only the new text.
                # print(f"DEBUG: [vLLM Delta] Yielding new text: '{newly_generated_text}'")
    ...
    # ===== ASYNC-NATIVE SPEECH GENERATOR (Unchanged) =====
    async def generate_speech_async(self, **kwargs):
        ...
        profiler = TTSDecoderProfiler() # <-- Create an instance
        ...
        # Pass the profiler to the decoder
...
class OrpheusModel:
    """
    Quantization-ready Orpheus LLM wrapper for TTS code emission.

    How to use with AWQ (recommended):
      model_path = "/models/orpheus-3b-0.1-ft-awq-w4g128-zp"
      m = OrpheusModel(
          model_path=model_path,
          tokenizer_path=model_path,
          quantization="awq",               # <- key line for vLLM
          dtype=torch.float16,              # activations dtype
          max_model_len=1024,
          gpu_memory_utilization=0.40,
          enforce_eager=True,
          max_num_seqs=128,
          # Optional (if your vLLM supports): kv_cache_dtype="fp8"
      )
    """
    ...
    def _format_prompt(self, prompt, voice="tara", model_type="larger"):
        # Keep prompt lean; encourage immediate codec tokens
        system = (
            "You are a TTS codec generator. Output ONLY <custom_token_N> codes with no explanations. Begin immediately."
        )
        adapted = f"{voice}: {prompt}" if voice else prompt
        prompt_tokens = self.tokenizer(system + "\n" + adapted, return_tensors="pt")
        start_token = torch.tensor([[128259]], dtype=torch.int64)
        end_tokens = torch.tensor([[128009, 128260, 128261, 128257]], dtype=torch.int64)
        all_input_ids = torch.cat([start_token, prompt_tokens.input_ids, end_tokens], dim=1)
        return self.tokenizer.decode(all_input_ids[0])
